import 'dart:async';

import 'package:bloc/bloc.dart';
import 'package:equatable/equatable.dart';
import 'package:groceryhelper/core/services/dialog_service.dart';
import 'package:groceryhelper/core/services/locator.dart';
import 'package:groceryhelper/core/services/talker_service.dart';
import 'package:groceryhelper/features/register/domain/entities/register_request.dart';
import 'package:groceryhelper/features/register/domain/usecases/register_usecase.dart';
import 'package:groceryhelper/features/register/domain/repositories/register_repository.dart';
import 'package:groceryhelper/features/register/data/repositories/register_repository_factory.dart';

part 'register_event.dart';
part 'register_state.dart';

class RegisterBloc extends Bloc<RegisterEvent, RegisterState> {
  late final RegisterUsecase _registerUsecase;

  RegisterBloc({RegisterRepository? registerRepository, bool useMock = true}) : super(RegisterInitial()) {
    // Создаем репозиторий локально, если не передан
    final repository = registerRepository ?? RegisterRepositoryFactory.create(useMock: useMock);

    // Создаем use case локально
    _registerUsecase = RegisterUsecase(registerRepository: repository);

    on<RegisterUserEvent>(_onRegisterUser);
  }

  Future<void> _onRegisterUser(RegisterUserEvent event, Emitter<RegisterState> emit) async {
    try {
      TalkerService.log('RegisterBloc: Starting registration process');

      DialogService.instance.showLoading();

      final registerRequest = RegisterRequest(username: event.username, email: event.email, password: event.password);

      final result = await _registerUsecase.execute(registerRequest);

      await Future.delayed(Duration(seconds: 10));

      // result.fold(
      //   (exception) {
      //     TalkerService.error('RegisterBloc: Registration failed', exception);
      //     DialogService.instance.showErrorDialog(title: 'Ошибка', message: exception.toString());
      //     event.completer.completeError(exception);
      //   },
      //   (userCredential) {
      //     TalkerService.log('RegisterBloc: Registration successful');
      //     DialogService.instance.showConfirmDialog(title: 'Регистрация успешна', message: 'Регистрация успешна');
      //     event.completer.complete(userCredential);
      //   },
      // );
    } catch (e) {
      TalkerService.error('RegisterBloc: Unexpected error', e);
      DialogService.instance.showErrorDialog(title: 'Ошибка', message: 'Неожиданная ошибка при регистрации');
      event.completer.completeError(e);
    }
  }

  @override
  Future<void> close() {
    TalkerService.log('RegisterBloc: Closing');
    return super.close();
  }
}
