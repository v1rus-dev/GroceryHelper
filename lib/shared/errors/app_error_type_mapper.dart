...
AppError mapExceptionToAppError(Object e, StackTrace st, {String? customMessage, AppErrorType? customType}) {
  ...
  if (customMessage != null && customType != null) {
    return AppError(message: customMessage, type: customType, cause: e, stackTrace: st);
  }

  // Если только customMessage, но не customType
  if (customMessage != null) {
    return AppError(message: customMessage, type: customType ?? AppErrorType.snackbar, cause: e, stackTrace: st);
  }

  // Если только customType, но не customMessage
  if (customType != null) {
    return AppError(message: _defaultMessageForException(e), type: customType, cause: e, stackTrace: st);
  }
  ...
  // Дефолтная логика
  return AppError(message: _defaultMessageForException(e), type: AppErrorType.snackbar, cause: e, stackTrace: st);
}
